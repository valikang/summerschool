...
void exchange_init(field *temperature, parallel_data *parallel)
{
    ...
    // TODO
		...
		MPI_Isend(temperature->data[1],temperature->ny+2,MPI_DOUBLE,parallel->nup, 11, MPI_COMM_WORLD, &parallel->requests[0]);
		MPI_Irecv(temperature->data[temperature->nx + 1],temperature->ny + 2, MPI_DOUBLE, parallel->ndown, 11,MPI_COMM_WORLD, &parallel->requests[1]);
    // Send to the down, receive from up
		MPI_Isend(temperature->data[temperature->nx],temperature->ny+2,MPI_DOUBLE,parallel->ndown,12,MPI_COMM_WORLD, &parallel->requests[2]);
		MPI_Irecv(temperature->data[0],temperature->ny+2,MPI_DOUBLE,parallel->nup,12,MPI_COMM_WORLD, &parallel->requests[3]);
    // TODO
...
{
		MPI_Waitall(4,&parallel->request,MPI_STATUSES_IGNORE);
}
